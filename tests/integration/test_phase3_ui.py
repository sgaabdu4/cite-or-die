...
def test_phase3_ui_wires_streaming_and_pdfjs(monkeypatch, tmp_path) -> None:
    ...
    assert 'type="module" src="/static/app.js?v=cfo-flow-v3"' in response.text
    ...
    assert "citation_viewer.js" in app_js
    ...
    assert "pdfjsLib.getDocument" in citation_viewer_js
    assert "GlobalWorkerOptions.workerSrc" in citation_viewer_js
    assert "renderPdfTextLayer" in citation_viewer_js
    assert "citationViewer.reset();" in app_js
    ...
    assert 'document.createElement("mark")' in citation_viewer_js
