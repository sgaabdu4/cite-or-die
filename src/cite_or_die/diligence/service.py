...
class DiligenceService:
    ...
    async def run_provider_assisted_review(
        ...
        context = self._provider_generation_context(
            deal,
            question,
            self._evidence_links_for_provider(knowledge_base, findings, insights, reports),
        ...
        ).inc(_approx_token_count(context.question, retrieved))
        ...
        report = self._provider_report_draft(
            deal,
    ...
    def _evidence_links_for_provider(
        self,
        knowledge_base: DiligenceKnowledgeBase,
