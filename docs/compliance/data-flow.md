...
10. Optional AI-assisted diligence runs only after the baseline review, sends pseudonymized cited diligence evidence chunks through the configured provider after prompt and retrieved-content guardrails, retries transient provider `429`/`5xx` responses briefly, and stores only verified cited report claims.
11. Provider HTTP, network, malformed-response, and citation failures return controlled API errors without raw provider bodies or raw model output.
12. Provider settings writes encrypt keys per tenant; tampered settings or `auth_secret` rotation return a controlled unreadable-config error until an admin deletes and recreates the config.
13. Audit events store allowlisted metadata and hash-chain integrity fields.
